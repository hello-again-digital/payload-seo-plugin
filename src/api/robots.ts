...
import type { PluginTypes, RobotsDocument } from '../types';
import { RobotsEntriesResponse } from '../types';
...
export const getRobotsTxt = async (req: PayloadRequest, pluginOptions: PluginTypes) => {
  try {
    ...
    })) as unknown as PaginatedDocs<RobotsDocument>;
    ...
    return Response.json(
      { error: 'An error occured when attempting to generate robots.txt' },
      { status: 500 },
    );
  }
};
...
export const getRobots = async (req: PayloadRequest) => {
  try {
    ...
    return Response.json(
      { error: 'An error occured when attempting to fetch robots entries' },
      { status: 500 },
    );
  }
};
